        ...
        <div class="col">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Sửa Tình Trạng</li>
                </ol>
            </nav>
        </div>
    </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0">Sửa tình trạng</h5>
                    </div>
                </div>
                <div class="card-body">
                    <?php if (!empty($errors)): ?>
                    ...
                    <?php endif; ?>
                <form action="index.php?model=tinhtrang&action=edit&id=<?= $tinhTrang['tinh_trang_id']; ?>" method="POST">
                    <div class="row mb-3">
                        <div class="col">
                            <label for="vi_tri_id" class="mr-2 mb-0" style="white-space: nowrap;">Tên Vị trí:</label>
                            <select id="vi_tri_id" name="vi_tri_id" class="form-control">
                                <option value="">Chọn vị trí</option>
                                <?php foreach ($baoTris as $baoTri): ?>
                                    <option value="<?= htmlspecialchars($baoTri['vi_tri_id']); ?>" <?= ($baoTri['vi_tri_id'] == $tinhTrang['vi_tri_id']) ? 'selected' : ''; ?>>
                        ...
                        <div class="col">
                            ...
                            <select id="schedule_id" name="schedule_id" class="form-control">
                                ...
                                <?php foreach ($baoTris as $lich): ?>
                                    <option value="<?= htmlspecialchars($lich['schedule_id']); ?>" 
                                            data-vi-tri-id="<?= htmlspecialchars($lich['vi_tri_id']); ?>"
                                            <?= ($lich['schedule_id'] == $tinhTrang['schedule_id']) ? 'selected' : ''; ?>>
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            ...
                            <textarea name="mo_ta_tinh_trang" id="mo_ta_tinh_trang" class="form-control" rows="3" placeholder="Mô tả tình trạng"><?= htmlspecialchars($tinhTrang['mo_ta_tinh_trang']) ?></textarea>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateScheduleOptions() {
        const selectedViTriId = viTriSelect.value;
        ...
        if (selectedViTriId && !scheduleSelect.querySelector(`option[data-vi-tri-id="${selectedViTriId}"]:checked`)) {
    ...
    // Gọi hàm này khi trang được tải để ẩn các lịch không phù hợp
    updateScheduleOptions();
});
</script>